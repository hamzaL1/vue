...
const jsmafRuntime = jsmaf as { loader_has_run?: boolean }

if (jsmafRuntime.loader_has_run) {
  throw new Error('loader already ran')
}
jsmafRuntime.loader_has_run = true
...
const themeFolder = (typeof CONFIG !== 'undefined' && typeof (CONFIG as any).theme === 'string') ? (CONFIG as any).theme : 'default'
...
if (!is_jailbroken) {
  const jb_behavior = (typeof CONFIG !== 'undefined' && typeof (CONFIG as any).jb_behavior === 'number') ? (CONFIG as any).jb_behavior : 0
